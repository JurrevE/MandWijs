    ...
    <>
      ...
      <div className="grid gap-5 lg:grid-cols-[15rem_minmax(0,1fr)]">
        ...
        <Card className="overflow-hidden shadow-none">
          {tab === "profile" && <section><div className="border-b border-mandwijs-line p-5 sm:p-7"><h2 className="text-xl font-black">Profielgegevens</h2><p className="mt-1 text-sm text-mandwijs-muted">De naam gebruiken we in je dashboard en weekmail.</p></div><div className="grid gap-5 p-5 sm:max-w-2xl sm:grid-cols-2 sm:p-7"><label className="block text-sm font-bold">Naam<input value={name} onChange={(event) => setName(event.target.value)} className="input-field mt-2" /></label><label className="block text-sm font-bold">E-mailadres<input value={userEmail} disabled className="input-field mt-2 bg-[#f4f6f5] text-mandwijs-muted" /></label><div className="sm:col-span-2"><h3 className="text-sm font-bold">Actieve locatie</h3><p className="mt-2 rounded-xl border border-mandwijs-line bg-[#f8faf9] p-4 text-sm">{profile.locationLabel} · {profile.radiusKm} km radius</p></div></div></section>}
          ...
          {tab === "security" && <section><div className="border-b border-mandwijs-line p-5 sm:p-7"><h2 className="text-xl font-black">Accountbeveiliging</h2><p className="mt-1 text-sm text-mandwijs-muted">{mode === "supabase" ? "Authenticatie en sessies worden beheerd door Supabase." : "Dit is een lokale demosessie zonder echt account."}</p></div><div className="space-y-3 p-5 sm:p-7"><div className="flex items-center gap-4 rounded-2xl border border-mandwijs-line p-4"><span className="grid size-10 place-items-center rounded-xl bg-[#edf4f1]"><KeyRound className="size-5 text-mandwijs-primary" /></span><span className="flex-1"><strong className="block text-sm">Wachtwoord</strong><span className="text-xs text-mandwijs-muted">{mode === "supabase" ? <Link href="/forgot-password" className="font-bold text-mandwijs-deep hover:underline">Stuur een beveiligde herstelmail</Link> : "Niet beschikbaar voor de demo."}</span></span><Badge tone={mode === "supabase" ? "success" : "neutral"}>{mode === "supabase" ? "Supabase" : "Demo"}</Badge></div><div className="rounded-2xl border border-[#f1d3d6] bg-[#fff7f8] p-4"><strong className="text-sm text-[#943941]">Account verwijderen</strong><p className="mt-1 text-xs leading-5 text-mandwijs-muted">Definitieve accountverwijdering vereist herauthenticatie en een aparte server-side beheeractie; deze knop is daarom nog niet geactiveerd.</p></div></div></section>}
          <div className="flex items-center justify-between border-t border-mandwijs-line bg-[#fbfcfb] px-5 py-4 sm:px-7"><span aria-live="polite" className={`text-xs font-bold ${persistenceError ? "text-[#a43d45]" : "text-mandwijs-primary"}`}>{persistenceError ?? (saved ? "Wijzigingen opgeslagen" : mode === "supabase" ? (databaseReady ? "Wijzigingen worden aan je account gekoppeld" : "Supabase-migratie vereist") : "Demo-instellingen worden lokaal bewaard")}</span><Button onClick={save}><Save className="size-4" /> Opslaan</Button></div>
        </Card>
      </div>
    </>
  );
