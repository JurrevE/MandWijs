    ...
    <>
      ...
      <div className="grid gap-5 xl:grid-cols-[1.2fr_.8fr]">
        <Card className="overflow-hidden shadow-none">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-mandwijs-line p-5"><div><h2 className="font-black">Synchronisaties</h2><p className="mt-1 text-xs text-mandwijs-muted">Logging, retries en importresultaten</p></div><Button variant="secondary" onClick={simulateSync} disabled={syncing}><RefreshCcw className={`size-4 ${syncing ? "animate-spin" : ""}`} /> {syncing ? "Bezig…" : "Demo-sync starten"}</Button></div>
          <div className="overflow-x-auto"><table className="w-full min-w-[42rem] text-left text-sm"><thead className="bg-[#f7faf8] text-xs text-mandwijs-muted"><tr><th className="px-5 py-3 font-bold">Provider</th><th className="px-5 py-3 font-bold">Status</th><th className="px-5 py-3 font-bold">Gestart</th><th className="px-5 py-3 font-bold">Geïmporteerd</th><th className="px-5 py-3 font-bold">Mislukt</th></tr></thead><tbody><tr className="border-t border-mandwijs-line"><td className="px-5 py-4 font-bold">DemoDataProvider</td><td className="px-5 py-4"><Badge tone="success">Geslaagd</Badge></td><td className="px-5 py-4 text-mandwijs-muted">10 aug, 08:15</td><td className="px-5 py-4">{demoOffers.length}</td><td className="px-5 py-4">0</td></tr><tr className="border-t border-mandwijs-line"><td className="px-5 py-4 font-bold">PrijsProfeetProvider</td><td className="px-5 py-4"><Badge tone="neutral">Niet geconfigureerd</Badge></td><td className="px-5 py-4 text-mandwijs-muted">—</td><td className="px-5 py-4">—</td><td className="px-5 py-4">—</td></tr></tbody></table></div>
        </Card>
      ...
      <Card className="mt-5 overflow-hidden shadow-none"><div className="border-b border-mandwijs-line p-5"><h2 className="font-black">Productmatches</h2><p className="mt-1 text-xs text-mandwijs-muted">Voorbeeld van de uitlegbare matchkwaliteit</p></div><div className="overflow-x-auto"><table className="w-full min-w-[45rem] text-left text-sm"><thead className="bg-[#f7faf8] text-xs text-mandwijs-muted"><tr><th className="px-5 py-3">Persoonlijk product</th><th className="px-5 py-3">Bronproduct</th><th className="px-5 py-3">Type</th><th className="px-5 py-3">Zekerheid</th><th className="px-5 py-3">Effectieve prijs</th></tr></thead><tbody>{demoOffers.slice(0, 6).map((offer, index) => <tr key={offer.id} className="border-t border-mandwijs-line"><td className="px-5 py-4 font-bold">{demoProducts[index % demoProducts.length].name}</td><td className="px-5 py-4 text-mandwijs-muted">{offer.product.name}</td><td className="px-5 py-4"><Badge tone={offer.product.isHouseBrand ? "warning" : "success"}>{offer.product.isHouseBrand ? "Huismerk alternatief" : "Exact product"}</Badge></td><td className="px-5 py-4">{offer.product.isHouseBrand ? "72%" : "96%"}</td><td className="px-5 py-4 font-bold">{formatEuro(offer.effectiveUnitPriceCents)}</td></tr>)}</tbody></table></div></Card>
    </>
  );
}
