...
export function OnboardingFlow() {
  ...
  const { profile, chains, mode, updateProfile, flushPersistence, toggleChain, products, addProduct, addToList } = useAppState();
    ...
    <main className="min-h-screen bg-[#f7faf8]">
      ...
      <div className="mx-auto grid max-w-6xl gap-8 px-5 pb-12 pt-6 sm:px-8 lg:grid-cols-[15rem_minmax(0,1fr)] lg:pt-14">
        ...
        <section className="surface-card mx-auto w-full max-w-2xl p-5 sm:p-9">
          ...
          {completionError && <p role="alert" className="mt-6 rounded-xl border border-[#efc9c9] bg-[#fff5f5] p-3 text-sm font-semibold text-[#a33f3f]">{completionError}</p>}
          <div className="mt-9 flex items-center justify-between border-t border-mandwijs-line pt-5"><Button variant="ghost" disabled={step === 0 || advancing} onClick={() => setStep(step - 1)}><ArrowLeft className="size-4" /> Terug</Button>{step < steps.length - 1 ? <Button disabled={advancing} onClick={next}>{advancing ? <LoaderCircle className="size-4 animate-spin" /> : null} Verder <ArrowRight className="size-4" /></Button> : <Button disabled={advancing} onClick={finish}>{advancing ? <LoaderCircle className="size-4 animate-spin" /> : null} Naar mijn overzicht <ArrowRight className="size-4" /></Button>}</div>
        </section>
      </div>
    </main>
  );
