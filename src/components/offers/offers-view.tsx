  ...
  return (
    <>
      ...
      <div className="mb-5 flex flex-col gap-3 sm:flex-row">
        <label className="relative flex-1"><Search className="absolute left-3.5 top-1/2 size-4 -translate-y-1/2 text-kopert-muted" /><input className="input-field pl-10" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Zoek in jouw aanbiedingen" /></label>
        <label className="relative sm:w-52"><select className="input-field appearance-none pr-9" value={chain} onChange={(event) => setChain(event.target.value)}><option value="all">Alle supermarkten</option>{demoChains.filter((item) => profile.enabledChainIds.includes(item.id)).map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select><SlidersHorizontal className="pointer-events-none absolute right-3.5 top-1/2 size-4 -translate-y-1/2 text-kopert-muted" /></label>
        <button onClick={() => setDealsOnly(!dealsOnly)} className={`min-h-12 rounded-xl border px-4 text-sm font-bold ${dealsOnly ? "border-kopert-deep bg-kopert-deep text-white" : "border-kopert-line bg-white text-kopert-muted"}`}><BadgePercent className="mr-2 inline size-4" />Alleen acties</button>
      </div>
      ...
      {offers.length === 0 ? <Card className="grid place-items-center px-5 py-16 text-center shadow-none"><Search className="size-8 text-kopert-muted" /><h2 className="mt-3 font-black">Geen resultaten</h2><p className="mt-1 text-sm text-kopert-muted">Pas je filters of zoekterm aan.</p></Card> : <div className="grid gap-4 md:grid-cols-2 2xl:grid-cols-3">
        {offers.map((offer) => {
          ...
          return <Card key={offer.id} className="overflow-hidden shadow-none">
            <div className="flex items-start gap-3 p-5 pb-4">
              ...
              <div className="min-w-0 flex-1"><div className="flex flex-wrap gap-1.5">{isDeal && <Badge tone="deal">{actionLabels[offer.actionType]}</Badge>}{option.matchType === "house_brand" ? <Badge tone="warning">Huismerk alternatief</Badge> : <Badge tone="success">{option.matchType === "exact" ? "Exact product" : "Vergelijkbaar product"}</Badge>}</div><h2 className="mt-2 line-clamp-2 text-sm font-extrabold leading-5">{offer.product.name}</h2><p className="mt-1 text-xs text-kopert-muted">voor jouw “{ownProduct?.name}”</p></div>
            </div>
            <div className="mx-5 rounded-2xl bg-[#f3f8f5] p-4">
              <div className="flex items-end justify-between"><span><small className="block text-xs font-bold text-kopert-muted">Effectief per stuk</small><strong className="mt-1 block text-2xl font-black tracking-[-.04em] text-kopert-deep">{formatEuro(offer.effectiveUnitPriceCents)}</strong></span>{isDeal && <span className="text-right"><small className="block text-[.68rem] text-kopert-muted">Normaal</small><span className="text-sm font-bold text-kopert-muted line-through">{formatEuro(offer.regularPriceCents)}</span></span>}</div>
              {offer.minimumQuantity > 1 && <div className="mt-3 grid grid-cols-2 gap-2 border-t border-kopert-line pt-3 text-xs"><span><span className="block text-kopert-muted">Je betaalt</span><strong>{formatEuro(offer.payableTotalCents)}</strong></span><span><span className="block text-kopert-muted">Minimaal kopen</span><strong>{offer.minimumQuantity} stuks</strong></span></div>}
            </div>
            <div className="flex items-center justify-between gap-3 px-5 py-4 text-xs text-kopert-muted"><span>{chainInfo.name}</span>{offer.loyaltyCardRequired && <span className="flex items-center gap-1 font-bold text-[#8a5718]"><CreditCard className="size-3.5" /> Klantenkaart vereist</span>}</div>
